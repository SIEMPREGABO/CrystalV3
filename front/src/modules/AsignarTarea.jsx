...
function AsignarTarea() {
  ...
  const { user } = useAuth();
  const { fechasproject,  iteracionactual, createTask, participants, requerimientos,  tareas , getProject, actualizarTareas} = useProject();
  const [tareasgrid, setTareasgrid] = useState([]);
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    const tareas = await actualizarTareas();
    setTareasgrid(tareas);
  });
